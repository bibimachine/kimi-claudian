...
const KIMI_AGENT_INVALID_SEGMENT_PATTERN = /[<>:"\\|?*]/;
...
export function validateKimiAgentName(name: string): string | null {
  ...
  for (const segment of segments) {
    ...
    if (segment.includes('\0') || KIMI_AGENT_INVALID_SEGMENT_PATTERN.test(segment)) {
...
class KimiAgentModal extends Modal {
  ...
  constructor(
    ...
    allAgents: KimiAgentDefinition[],
    onSave: (agent: KimiAgentDefinition) => Promise<void>,
  ) {
    super(app);
  ...
  onOpen() {
    this.setTitle(this.existing ? 'Edit Kimi code CLI subagent' : 'Add Kimi code CLI subagent');
    ...
    new Setting(contentEl)
      .setName('Name')
      .setDesc('Kimi code CLI agent name. Use slash-separated segments for nested agents.')
      .addText((text) => {
    ...
    new Setting(contentEl)
      .setName('Description')
      .setDesc('When kimi code CLI should use this subagent')
      .addText((text) => {
    ...
    saveBtn.addEventListener('click', () => {
      ...
      const agent: KimiAgentDefinition = {
        name,
        description,
        prompt,
        mode: 'subagent',
...
export class KimiAgentSettings {
  ...
  private storage: KimiAgentStorage;
  ...
  async render(): Promise<void> {
    ...
    headerEl.createSpan({ text: 'Kimi code CLI subagents', cls: 'claudian-sp-label' });
    ...
    if (visibleAgents.length === 0) {
      ...
      emptyEl.setText('No kimi code CLI subagents in vault. Click + to create one.');
      return;
    }
  ...
  private openModal(existing: KimiAgentDefinition | null): void {
    ...
    const modal = new KimiAgentModal(
      this.app,
      existing,
      this.agents,
      async (agent) => {
