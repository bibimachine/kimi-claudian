...
import {
  KIMI_AGENT_KNOWN_KEYS,
  type KimiAgentDefinition,
} from '../types/agent';

export const KIMI_AGENT_PATH = '.kimi/agent';
export const KIMI_AGENTS_PATH = '.kimi/agents';
const KIMI_AGENT_SCAN_PATHS = [
  KIMI_AGENTS_PATH,
  KIMI_AGENT_PATH,
] as const;
const KIMI_DEFAULT_AGENT_SAVE_PATH = KIMI_AGENT_PATH;
const KIMI_AGENT_PERSISTENCE_PREFIX = 'kimi-agent';

export interface KimiAgentLocation {
  filePath: string;
}

export function createKimiAgentPersistenceKey(
  location: KimiAgentLocation,
): string {
  return `${KIMI_AGENT_PERSISTENCE_PREFIX}:${encodeURIComponent(normalizeVaultPath(location.filePath))}`;
}

export function parseKimiAgentPersistenceKey(
  persistenceKey?: string,
): KimiAgentLocation | null {
  if (!persistenceKey) {
    return null;
  }
  ...
  if (prefix !== KIMI_AGENT_PERSISTENCE_PREFIX || !encodedRelativePath) {
    return null;
  }
  ...
  return decoded.endsWith('.md')
    ? { filePath: `${KIMI_AGENTS_PATH}/${decoded}` }
    : null;
}
...
export class KimiAgentStorage {
  ...
  async load(agent: KimiAgentDefinition): Promise<KimiAgentDefinition | null> {
    ...
    try {
      ...
      return parseKimiAgentMarkdown(content, filePath);
    } catch {
      return null;
    }
  }
  ...
  async save(agent: KimiAgentDefinition, previous?: KimiAgentDefinition | null): Promise<void> {
    ...
    const content = serializeKimiAgentMarkdown(agent);
  ...
  private resolveCurrentPath(agent: KimiAgentDefinition): string {
    const persistedLocation = parseKimiAgentPersistenceKey(agent.persistenceKey);
    if (persistedLocation) {
      return persistedLocation.filePath;
    }

    return `${KIMI_DEFAULT_AGENT_SAVE_PATH}/${agent.name}.md`;
  }

  private resolveTargetPath(
    agent: KimiAgentDefinition,
    previous?: KimiAgentDefinition | null,
  ): string {
      ...
      try {
        ...
        for (const filePath of files) {
          ...
          try {
            ...
            const agent = parseKimiAgentMarkdown(content, filePath);
            if (!agent) continue;
...
export function parseKimiAgentMarkdown(
  ...
  const result: KimiAgentDefinition = {
    name,
    description,
    prompt: parsed.body.trim(),
    persistenceKey: createKimiAgentPersistenceKey({
  ...
  for (const [key, value] of Object.entries(frontmatter)) {
    if (!KIMI_AGENT_KNOWN_KEYS.has(key)) {
...
function normalizeMode(value: unknown): KimiAgentDefinition['mode'] | undefined {
...
function isSupportedAgentFilePath(filePath: string): boolean {
  return KIMI_AGENT_SCAN_PATHS.some((rootPath) => filePath.startsWith(`${rootPath}/`))
